...
struct CharacterRNNPostagModel : public PostagModel {
  ...
  void get_word_reprs(const std::vector<std::string> & words,
                      std::vector<dynet::Expression> & words_reprs) override {
    ...
    for (unsigned i = 0; i < n_words; ++i) {
      ...
      words_reprs[i] = dynet::concatenate({ payload.first, payload.second, embed_input.get_output(embeddings[i]) });
    }
  }

  dynet::Expression get_score(dynet::Expression & word_repr,
                              unsigned prev_tag) const {
  ...
  void decode(const std::vector<std::string> & words, std::vector<std::string> & tags) override {
    ...
    unsigned n_words = words.size();
    std::vector<dynet::Expression> word_reprs;
    get_word_reprs(words, word_reprs);
    ...
    for (unsigned i = 0; i < n_words; ++i) {
      ...
      dynet::Expression logits = get_score(
        dynet::concatenate({ payload.first, payload.second, pos_embed.embed(prev_label) })
      )));
      std::vector<float> scores = dynet::as_vector((char_embed.cg)->get_value(logits));
      unsigned label = std::max_element(scores.begin() + 1, scores.end()) - scores.begin();
  ...
  dynet::Expression objective(const Instance & inst) override {
    ...
    std::vector<dynet::Expression> words_reprs(n_words);
    get_word_reprs(words, words_reprs);

    word_rnn.add_inputs(words_reprs);
    std::vector<dynet::Expression> losses(n_words);
    unsigned prev_label = root_pos_id;
    for (unsigned i = 0; i < n_words; ++i) {
      auto payload = word_rnn.get_output(i);
      dynet::Expression logits = dense2.get_output(dynet::rectify(dense1.get_output(
        dynet::concatenate({ payload.first, payload.second, pos_embed.embed(prev_label) })
      )));
