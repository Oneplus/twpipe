...
std::string url_start_1("(?:https?://|\bwww\.)");
std::string common_tlds("(?:com|org|edu|gov|net|mil|aero|asia|biz|cat|coop|info|int|jobs|mobi|museum|name|pro|tel|travel|xxx)");
std::string cc_tlds("(?:ac|ad|ae|af|ag|ai|al|am|an|ao|aq|ar|as|at|au|aw|ax|az|ba|bb|bd|be|bf|bg|bh|bi|bj|bm|bn|bo|br|bs|bt|"
...
std::string url_start_2("\\b(?:[A-Za-z\\d-])+(?:\\.[A-Za-z0-9]+){0,3}\\.(?:" + common_tlds + "|" + cc_tlds + ")(?:\\." + cc_tlds + ")?(?=\\W|$)");
std::string url_body("(?:[^\\.\\s<>][^\\s<>]*?)?");
